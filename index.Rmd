...
In this example we'll get a list of all users who've never posted a question, order it by displayname, and show the only the first 25 results.
...
Another common form of subquery uses an IN construct. (need better example)
```SQL
SELECT displayname
FROM users
WHERE userid IN (
